...
sub count_prob{
  ...
  while(my($key,$value) = each(%destcount)){
    ...
    my $origin_prob = $origin_split[1]+0;#cast as a double
    my @dest_split = split(/_/,$dest);
    my $dest_page = int $dest_split[0];
    # print "$dest_lvalue\n";
    my $dest_actionVct = int $dest_split[1];
    my $dest_prob = $dest_split[1]+0;#cast as a double
    my $reward=0;
    ...
    if($action=~ /^Quiz/ or $action=~ /^Video/ or $action=~ /^Hint/){
      $reward += -0.04;
      # $reward = -0.1;
    }
    ...
    if($origin_prob > $dest_prob){
      #this condition doesn't happen for now.
      $reward += -0.1;
      #$reward = -0.5;
    }
    if($origin_prob == $dest_prob){
    ...
    #do not print out if the transition is from terminal state = 8.5
    if($origin_prob < 8.5){
