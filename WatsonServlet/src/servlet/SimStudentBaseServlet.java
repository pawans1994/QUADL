...
import interaction.ModelTracerBackend;
import interaction.SAI;
import interaction.SimBackend;
import interaction.SimStTutoringBackend;
...
public class SimStudentBaseServlet extends HttpServlet implements ServletContextListener{
	...
	private Backend ab;
	public static String watsonFolder;
	...
	public void init(ServletConfig servletConfig)
	{
		...
		watsonFolder = servletConfig.getServletContext().getInitParameter("folderPath");
		jarFolder = servletConfig.getServletContext().getInitParameter("jarFolder");
		System.out.println(" watson Folder : "+watsonFolder+" jarFolder : "+jarFolder);
...
//		System.out.println("Finished processing post request");
	...
	public void doOptions(HttpServletRequest req, HttpServletResponse resp)
	    ...
	    resp.setHeader("Access-Control-Allow-Headers", "access-control-allow-origin, ctatsession, content-type,X-CSRFToken");
	...
	private String getResponse(String request)
	{
		...
		for(RequestMessage rm : rms){
			...
			if(rm instanceof SetPreferenceRequest){//same as new StartStateResponse().toXML()
				...
				if(b instanceof SimStTutoringBackend)
					((SimStTutoringBackend)b).setsimstSession(session);
...
//				giveWMEs(b,spr.getWmes());
		...
		System.out.println(" Gonna form a response");
	...
	private void sendResponse(HttpServletResponse responseMech, String responseText)
	{
		try{
			...
			responseMech.setHeader("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept,ctatsession,X-CSRFToken");
	...
	public void giveWMEs(Backend b, String brd){
		...
		while(brd != null && brd.indexOf(start) != -1){
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		response.setHeader("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept,session_id,X-CSRFToken");
	...
	public void sendSkillMessage(String skillname, String session){
		if(skillname == null)
			return;
		String message = ResponseCreator.createTutorPerformedResponseSkill(skillname);
		messMan.addMessage(session, message);
	}
	
	public void sendSkillCountMessage(String skillname, int count, String session){
		if(skillname == null)
			return;
		String message = ResponseCreator.createTutorPerformedResponseSkill(skillname,count);
		messMan.addMessage(session, message);
	}
	public void sendProblemMessage(String problem, String session){
		if(problem == null)
			return;
		String message = ResponseCreator.createTutorPerformedResponseProblemList(problem);
		messMan.addMessage(session, message);
	}
	/**
	...
	{
		System.out.println ("ServletContextListener destroyed "+event.getClass());
	}
	/**
	 *
	 */
