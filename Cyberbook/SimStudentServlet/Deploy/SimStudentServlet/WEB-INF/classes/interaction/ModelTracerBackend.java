...
public class ModelTracerBackend extends Backend {
	...
	*/
	...
	@Override
	//originally this method returned void
	public String processInterfaceEvent(InterfaceEvent ie) {
		...
		switch(ie.getType()){
		case SAI:
			result += processSAI(ie.getEvent(),ie.getTransactionID());
			break;
		}
		return result;
	}
	...
	//originally this method returned void
	private String processSAI(SAI sai,String transactionID) {
		...
		if(action.equals("UpdateTextArea") || action.equals("UpdateTextField")){//text area typed into
			result += processUpdateTextArea(sai,transactionID);
		}else if(action.equals("ButtonPressed")){//button is pressed
			result += processButtonPressed(sai,transactionID);
		}
		return result;
	}
	
	//originally this method returned void
	private String processUpdateTextArea(SAI sai,String transactionID) {
		MTSAI mtsai = makeTracerSai(sai);
		MTResult result = null;
		try {
			...
			if (selection != null) {
				...
				if (at != null && at.getIsEnabled()) {
					...
					result = this.solver.sendSAI(mtsai);
					...
					//Returning the result computed by the Model Tracer Backend
					
					//Uncomment this if you want the normal SimStudentServlet to start
	...
	private String processButtonPressed(SAI sai,String transactionID) {
		MTResult result = null;
		try {
			if (sai.getFirstSelection().equals(HINT_BUTTON_NAME)) {
				ArrayList<String> results = this.solver.getHint();
				...
				if (results!=null)
					...
					sendHintMessage(results);
				} else if (sai.getFirstSelection().equals(DONE_BUTTON_NAME)){
				result = this.solver.sendSAI(makeTracerSai(sai));
				
				//uncomment this to get the normal behavior of the SimStudentServlet
	...
	private MTSAI makeTracerSai(SAI sai) {
		System.out.println("SAI Format - "+formatSelection(sai.getFirstSelection())+" | "+sai.getFirstAction()+" | "+sai.getFirstInput());
