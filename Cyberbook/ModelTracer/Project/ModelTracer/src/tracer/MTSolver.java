...
public class MTSolver {
	...
	public MTSolver(String init, String types, String production) throws JessException {
		...
		System.out.println("Adding CLP files");
		this.engine.batch(types);
		System.out.println("Adding Init files");
		this.engine.batch(init);
		System.out.println("Adding Production files");
	...
	public MTResult sendSAI(MTSAI sai) throws MTException, JessException {
		...
		while(it.hasNext()) {
			...
			try {
				...
				if (i==0){
					System.out.println("Gonna throw exception");
					throw new MTException(MTExceptionType.FAIL, "Emtpy agenda");
				}
				
				// if here then student input is correct
				//if (logProductionRules)
				    recentRuleFired = it.next().toString();
					MTLog("Fired production rule" +recentRuleFired);
				
				this.hintFunction.resetHint();
				finishSending();
				return new MTResult(MTResultType.CORRECT, sai.toString(),recentRuleFired);
			} catch (MTException e) {
				if (e instanceof MTException) {
					System.out.println(" E : "+e.getType()+ " Message : "+e.getMessage());
					switch(e.getType()) {
					case BUGGYFAIL:
					case FAIL:
						tutorSAI = e.getMessage();
						System.out.println(" Switch case : "+tutorSAI);
						if(tutorSAI.length() > 0 && !tutorSAI.equals("Emtpy agenda")){
						    String[] tutor = tutorSAI.split(":");
						    String tutorInput = "";
						    Class inputMatcher;
							try {
								System.out.println(" Input Matcher : "+this.getInputMatcher());
								inputMatcher = Class.forName(this.getInputMatcher());
							    FeaturePredicate inputMatcherInstance = (FeaturePredicate)inputMatcher.newInstance();
							    Vector args = new Vector();
							    if(tutor != null && tutor.length > 0)
							    	tutorInput = tutor[tutor.length-1];
							    args.add(tutorInput.trim());
							    args.add(sai.getInput());
							    System.out.println(" Tutor input : "+tutorInput+" student input : "+sai.getInput());
							    String correct = inputMatcherInstance.apply(args);
					...
					case BUGGYMATCH:
						    recentRuleFired = it.next().toString();
							MTLog("Matched buggy production rule: " +recentRuleFired);
						finishSending();
		...
		finishSending();
		return new MTResult(MTResultType.INCORRECT, sai.toString(),recentRuleFired);
	}
	...
	public ArrayList<String> getHint() throws JessException, MTException {
		...
		while(it.hasNext()) {
			recentRuleFired = it.next().toString();
			MTLog("Finding hint at production rule " + recentRuleFired);
			try {
...
//				MTLog("reset problem fact");
	...
	public void resetWM(String initFile,String wmeType,String productionRules){
		try {
			this.engine.clear();
			this.engine.batch(wmeType);
			this.engine.batch(initFile);
			this.engine.batch(productionRules);
		} catch (JessException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
