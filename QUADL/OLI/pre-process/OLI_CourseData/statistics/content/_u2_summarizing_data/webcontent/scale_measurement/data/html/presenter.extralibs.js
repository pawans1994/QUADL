...
(function() {
    ...
    $.event.special.swipeall = {
		...
		InDirDistanceThreshold: 30,  // Swipe horizontal displacement must be more than this.

		OffDirDistanceThreshold: 75,  // Swipe vertical displacement must be less than this.
        setup: function() {
            var thisObject = this;
            var $this = $(thisObject);
            $this.bind(touchStartEvent, function(event) {
                ...
                function moveHandler(event) {
					...
					triggerCustomEvent( thisObject, "swipemove", $.Event( "swipemove", { event: event, swipestop: stop}) );
                ...
                $this
                    ...
                    if (start && stop) {
                        if (stop.time - start.time < 1000) {
							/*if(Math.abs(start.coords[1] - stop.coords[1]) > $.event.special.swipeall.InDirDistanceThreshold &&
                                Math.abs(start.coords[0] - stop.coords[0]) < $.event.special.swipeall.OffDirDistanceThreshold) {

								var dir = start.coords[1] > stop.coords[1] ? "swipeup" : "swipedown";
								triggerCustomEvent( thisObject, "swipeall", $.Event( "swipeall", { target: event.target, swipestart: start, swipestop: stop , direction:dir}) );
							}
							else */if(Math.abs(start.coords[0] - stop.coords[0]) > $.event.special.swipeall.InDirDistanceThreshold &&
                                Math.abs(start.coords[1] - stop.coords[1]) < $.event.special.swipeall.OffDirDistanceThreshold) {

								var dir = start.coords[0] > stop.coords[0] ? "swipeleft" : "swiperight";
								triggerCustomEvent( thisObject, "swipeall", $.Event( "swipeall", { target: event.target, swipestart: start, swipestop: stop , direction:dir}) );
							}
                        }
                    }
