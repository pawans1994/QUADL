...
function MTMDisplayMenu(trackingpage) {
...
//		if(MTMTrack) { MTMTrackedItem = MTMTrackExpand(menu); }
		if(MTMTrack) { MTMTrackedItem = MTMTrackExpand(menu, trackingpage); }
...
function MTMTrackExpand(thisMenu, trackingpage) {
	var i, targetPath, targetLocation;
	var foundNumber = false;
	for(i = 0; i < thisMenu.items.length; i++) {
//alert("trackingpage is " + trackingpage + " and thisMenu.items.url is " + thisMenu.items[i].url);
...
//	} else if(MTMTrackedItem && thisItem.number == MTMTrackedItem) {
	if(MTMTrackedItem && thisItem.number == MTMTrackedItem) {
        	if(MTMTrackedCookieName) {
